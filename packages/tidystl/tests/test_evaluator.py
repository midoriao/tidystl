...
from tidystl import EvaluationResult, evaluate, list_backends, robustness, use
...
class TestBackendRegistry:
    ...
    def test_names_returns_sorted_registered_names(self) -> None:
        ...
        registry = BackendRegistry()
        registry.register(_Dummy("zeta"))  # type: ignore[arg-type]
        registry.register(_Dummy("alpha"))  # type: ignore[arg-type]
        assert registry.names() == ["alpha", "zeta"]

    def test_list_backends_uses_given_registry(self) -> None:
        class _Dummy:
            name = "solo"
        ...
        registry = BackendRegistry()
        registry.register(_Dummy())  # type: ignore[arg-type]
        assert list_backends(registry) == ["solo"]

    def test_list_backends_default_registry_has_native(self) -> None:
        # The tidystl default registry always has at least the built-in native
        # backend; plugin backends appear only after tidystl.use(...).
        assert "native" in list_backends()

    def test_use_registers_plugin_module(self) -> None:
        class _Dummy:
            name = "dummy"
        ...
        registry = BackendRegistry()
        use(_Plugin, registry=registry)
        assert list_backends(registry) == ["dummy"]

    def test_use_registers_backend_iterable(self) -> None:
        class _Dummy:
            name = "dummy"
        ...
        registry = BackendRegistry()
        use([_Dummy()], registry=registry)  # type: ignore[list-item]
        assert list_backends(registry) == ["dummy"]

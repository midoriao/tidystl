...
Covers the package docs ``usage.md`` (user manual) and ``reference.md`` (API
reference). Each block must be self-contained (its own imports and data) so
that readers can copy-paste it verbatim. Blocks that import torch or
tidystl_compat are skipped when those packages are not installed, but are still
compiled so syntax errors surface everywhere. Illustrative, non-runnable
fragments must use a non-``python`` fence (``text``, ``bash``, ...) to stay out
of this suite.
"""
...
def test_usage_doc_block_executes(block_id: str, source: str) -> None:
    ...
    if "tidystl_compat" in source:
        pytest.importorskip("tidystl_compat")

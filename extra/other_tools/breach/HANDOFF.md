...
real-Breach column of the E1 divergence matrix. Everything else in the campaign runs without it; the matrix
marks the missing cells `pending` until the CSVs land.

## What to run (on the MATLAB machine)

```sh
git clone https://github.com/midoriao/tidystl
...
- `generate_ground_truth` regenerates the 37 regression cases in
  `packages/tidystl-compat/tests/breach_ground_truth.jsonl` AND calls
  `generate_e1_column`, which writes the E1 column (21 CSVs + `breach_meta.txt`)
  to `extra/experiments/e1_divergence/cache/breach_column/`.
...
- `packages/tidystl-compat/tests/breach_ground_truth.jsonl` should regenerate with
  the same values as the recorded baseline (JSON number formatting may differ
  slightly from the original conversion, but the compat tests must still pass).
  A genuine value diff means a different Breach/MATLAB version than the recorded
  baseline -- do not discard it; commit nothing under `tests/` and report the
  diff instead.
...
```sh
git pull
make -C extra/experiments/e1_divergence report
```

`aggregate.py` imports the Breach column directly from `breach_column/`,
preferring those CSVs over the recorded regression fixtures (provenance
